...
1. **部署平台未配置环境变量**
   - 部署平台（如 Coze FaaS）要求手动配置环境变量
   - 即使启动脚本已修复，如果平台没有提供任何数据库连接信息，仍会失败
...
2. **或使用平台提供的数据库集成**
   - 如果平台支持数据库集成，创建 PostgreSQL 数据库实例
   - 平台会自动设置数据库连接环境变量

#### 详细指引

请参考新创建的快速配置指南：
- **`docs/DEPLOYMENT_ENV_QUICK_GUIDE.md`** - 部署环境变量配置快速指南
...
1. ⚠️ 在部署平台配置 `DATABASE_URL` 环境变量
2. ⚠️ 在部署平台配置 `JWT_SECRET` 环境变量

### 部署流程

```
代码修复 ✅ → 环境变量配置 ⚠️ → 重新部署 → 成功部署
```

**下一步**: 按照 `docs/DEPLOYMENT_ENV_QUICK_GUIDE.md` 配置环境变量，然后重新部署。
